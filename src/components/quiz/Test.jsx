...
import "./Test.css";
import { ArrowBigRightDash, Settings, TriangleAlert, CheckCircle, XCircle } from "lucide-react";
...
const Test = ({ data }) => {
  ...
  const [questions, setQuestions] = useState(preguntasAleatorias(npreguntas, data, preguntaAleatoria));
    ...
    if (!lock) {
      if (question.ans === ans) {
        ...
        if (question.ans === 0) {
          ...
          if (question.ans >= 1 && question.ans <= 5 && option_array[question.ans - 1]?.current) {
  ...
  const renderOpciones = (isMobile = false) => {
    ...
    const renderOrder = [0, 1, 2, 3, 4].map(i => (i + numero) % opciones.length);
    ...
    <div className="quiz-wrapper min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="hidden md:grid md:grid-cols-12 md:min-h-screen">
        ...
        <aside className="col-span-2 p-6 pt-20 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border-r border-slate-200 dark:border-slate-700 overflow-y-auto">
          <div className="sticky top-20 space-y-6">
            <div>
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Módulo</span>
              <h1 className="text-lg font-semibold text-slate-900 dark:text-white mt-1">{question.asignatura}</h1>
            </div>
            <div className="text-sm text-slate-600 dark:text-slate-300">
              <p>{question.tema}</p>
              <p className="text-slate-400 dark:text-slate-500 text-xs mt-2">{npreguntas} preguntas</p>
            </div>
            <Separator />
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className={`quiz-score quiz-score-correct ${scoreFlash === "correct" ? "animate-pulse-success" : ""}`}>
                  <CheckCircle size={18} /><span>{score}</span>
                </div>
        ...
        <main className="col-span-8 p-4 md:p-8 flex flex-col max-h-screen overflow-hidden">
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Pregunta {index + 1} de {npreguntas}</span>
              <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{Math.round(progressPercent)}%</span>
            </div>
          ...
          <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl shadow-xl border border-slate-200/50 dark:border-slate-700/50 rounded-2xl">
                  ...
                  {result ? (
                    <div className="flex flex-col items-center justify-center text-center space-y-6 py-8">
                      <motion.div
                        ...
                        {score}
                        <span className="text-2xl text-slate-400">/{npreguntas}</span>
                      </motion.div>
                      <p className="text-lg text-slate-600 dark:text-slate-300">Respuestas correctas</p>
                      <div className="px-6 py-3 bg-slate-100 dark:bg-slate-700 rounded-xl">
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                          Tiempo: <span className="font-semibold text-slate-900 dark:text-white">{temporizadorRef.current?.getTime()}</span>
                        </p>
                      </div>
                      <div className="flex gap-3 flex-wrap justify-center">
                        <Button className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-6 rounded-xl font-semibold transition-all hover:scale-105" onClick={reset}>
                          <ArrowBigRightDash /> Repetir
                        </Button>
                        {mal !== 0 && (
                          <Button className="bg-slate-200 hover:bg-slate-300 text-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 px-6 py-6 rounded-xl font-semibold transition-all hover:scale-105" onClick={resetErrores}>
                            Revisar errores ({mal})
                          </Button>
                        )}
                      </div>
                    ...
                    <div className="h-full flex flex-col">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="quiz-question-number">Pregunta #{question.id}</span>
                        <Separator orientation="vertical" className="h-4" />
                        <span className="text-sm text-slate-500">{question.tema}</span>
                      </div>
            ...
            {!result && (
              <CardFooter className="p-6 pt-0">
                <Button className="w-full bg-slate-900 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 text-white px-6 py-6 rounded-xl font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]" onClick={next} disabled={!lock}>
        ...
        <aside className="col-span-2 p-6 pt-20 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border-l border-slate-200 dark:border-slate-700 overflow-y-auto">
          <div className="sticky top-20 space-y-6">
            ...
            <Button className="w-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-xl py-3" onClick={() => setOpen(true)}>
      ...
      <div className="md:hidden flex flex-col min-h-screen">
        <header className="sticky top-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="quiz-score quiz-score-correct"><CheckCircle size={16} /><span className="text-sm">{score}</span></div>
              <div className="quiz-score quiz-score-wrong"><XCircle size={16} /><span className="text-sm">{mal}</span></div>
            </div>
            <Button size="icon" variant="ghost" onClick={() => setOpen(true)} className="rounded-full"><Settings size={20} /></Button>
          </div>
        ...
        <main className="flex-1 p-4 pb-24 overflow-y-auto">
          <Card className="bg-white dark:bg-slate-800 shadow-lg rounded-2xl border-0 min-h-[400px]">
            <CardContent className="p-5 h-full flex flex-col">
              <AnimatePresence mode="wait">
                <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-4 flex-1 flex flex-col">
                  {result ? (
                    <div className="text-center py-8 space-y-4">
                      <div className="text-5xl font-bold text-emerald-500">{score}<span className="text-xl text-slate-400">/{npreguntas}</span></div>
                      <p className="text-slate-600 dark:text-slate-300">respuestas correctas</p>
                      <div className="flex flex-col gap-2">
                        <Button className="w-full bg-emerald-500" onClick={reset}>Repetir</Button>
                        {mal !== 0 && <Button variant="outline" onClick={resetErrores}>Revisar errores</Button>}
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col flex-1">
                      <div className="flex items-center gap-2 text-xs text-slate-500">
                        <span className="quiz-question-number">#{question.id}</span>
                        <span>{question.asignatura}</span>
                      </div>
                      <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{question.question}</h2>
            ...
            {!result && (
              <CardFooter className="p-4 pt-2">
                <Button className="w-full bg-slate-900 text-white dark:bg-white dark:text-slate-900 py-4 rounded-xl font-semibold" onClick={next} disabled={!lock}>
        ...
        <footer className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between z-40 md:hidden">
          <Temporizador isRunning={isRunning} setIsRunning={setIsRunning} ref={temporizadorRef} />
          <span className="text-sm text-slate-500">{index + 1}/{npreguntas}</span>
        </footer>
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <div className="w-80 flex flex-col items-center p-4">
          <Settings size={40} className="text-slate-600 dark:text-slate-300 mb-4" />
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Configuración</h3>
          <Separator className="w-full mb-4" />
          <div className="w-full space-y-4">
            <div className="flex items-center justify-between">
              <Label htmlFor="preguntas-random" className="text-sm font-medium">Preguntas Aleatorias</Label>
              <Switch id="preguntas-random" checked={preguntaAleatoria} onCheckedChange={handlePreguntasChange} />
            </div>
          </div>
          <Button className="w-full mt-6 bg-emerald-500 hover:bg-emerald-600" onClick={handleSubmit}>Guardar</Button>
        </div>
      </Modal>
      ...
      <Modal open={openAlert} onClose={() => setOpenAlert(false)}>
        <div className="text-center w-64 py-4">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Pregunta con respuesta cuestionable. Consultá en documentation.</p>
          <Button className="w-full bg-slate-900 dark:bg-white" onClick={() => setOpenAlert(false)}>Continuar</Button>
        </div>
      </Modal>
    </div>
  );
